...
interface Job { id: string | number; title: string; company: string; is_featured?: boolean; description: string; tags?: string[]; url: string; }
            ...
            <>
              ...
              {filteredBoards.length === 0 && (
                <div className="rounded-2xl border border-dashed border-purple-200 bg-white p-10 text-center shadow-sm">
                  ...
                  <h3 className="mb-2 text-lg font-semibold text-foreground">No board matches that search</h3>
                  <p className="mx-auto mb-5 max-w-md text-sm leading-6 text-muted">Try another keyword, or reset the filter to see every curated Web3 job board.</p>
                  <button onClick={() => setSearchTerm('')} className="rounded-full bg-foreground px-5 py-3 text-sm font-semibold text-white">View all boards</button>
                </div>
              )}
