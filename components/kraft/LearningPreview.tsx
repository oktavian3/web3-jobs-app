...
const previews = [
  ...
  },
  {
    ...
    body: "You are 58% ready. Focus on reporting and crisis handling.",
    meta: ["Community & Growth", "Product Ops", "Content"],
  },
  {
    label: "Glossary",
    title: "TVL",
  ...
  },
  {
    ...
    body: "Role-specific project, documented result, case study, tailored application.",
    meta: ["3 complete", "2 remaining", "Next action"],
  },
];
...
export default function LearningPreview() {
  ...
  return (
    <div className="rounded-[28px] border border-border bg-white p-3 shadow-blue">
        ...
        {previews.map((item, index) => (
          <button
            ...
            className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-extrabold transition ${
              active === index ? "bg-blue-600 text-white shadow-soft" : "text-muted hover:bg-white hover:text-ink"
            }`}
          >
            {item.label}
          </button>
      ...
      <div className="mt-3 rounded-3xl bg-[linear-gradient(135deg,#f7fbff,#e8f2ff)] p-6 sm:p-8">
        <span className="rounded-full bg-white px-3 py-1 text-xs font-extrabold text-blue-700 shadow-soft">{preview.label}</span>
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-3">
          {preview.meta.map((item) => (
            <div key={item} className="rounded-2xl border border-white bg-white/80 p-4 shadow-soft">
